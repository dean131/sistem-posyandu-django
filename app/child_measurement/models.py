...
class ChildMeasurement(models.Model):
    ...
    def calculate_length_and_height_for_age(self):
            ...
            if gender == "M":
                if child_age < 24:
                    queryset = AnthropometricStandard.objects.filter(
                        measurement_type="m_length_for_age", index__lt=24
                    )
                else:
                    queryset = AnthropometricStandard.objects.filter(
                        measurement_type="m_height_for_age", index__gte=24
                    )
            else:
                if child_age < 24:
                    queryset = AnthropometricStandard.objects.filter(
                        measurement_type="f_length_for_age", index__lt=24
                    )
                else:
                    queryset = AnthropometricStandard.objects.filter(
                        measurement_type="f_height_for_age", index__gte=24
                    )
            return queryset
        ...
        if self.age_in_month < 24:
            ...
            # delete old image
            if self.child.growthchart.legth_for_age_chart_0_24:
                self.child.growthchart.legth_for_age_chart_0_24.delete()
            # save new image
            self.child.growthchart.legth_for_age_chart_0_24.save(
                f"{self.child.id}_legth_for_age_chart_0_24.png", content_file
            )
            plt.close()
            figure.close()
        else:
